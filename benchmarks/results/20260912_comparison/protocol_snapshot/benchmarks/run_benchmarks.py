...
def main():
    ...
    parser.add_argument('--max-output-tokens', type=int, choices=range(1, 8193), metavar='1..8192',
                        help='Override the total generation budget equally across selected cases (including reasoning).')
    ...
    metadata['source_sha256'] = {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
        for p in sorted(BENCH_DIR.glob('cases_*.json')) + sorted(BENCH_DIR.glob('*.py')) + sorted((ROOT / 'app').glob('*.py'))}
