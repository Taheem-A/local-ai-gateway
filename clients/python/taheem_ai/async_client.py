...
class AsyncAI:
    ...
    async def tool_turn(
        ...
        *,
        ...
        """Run one async tool-capable model turn without executing requested tools."""
    ...
    async def run_tools_once(
        ...
        *,
        ...
        """Execute at most one authorized async tool round, then force text synthesis."""

        messages: list[dict[str, Any]] = [{"role": "user", "content": prompt}]
        first = await self.tool_turn(
            messages,
            registry,
            quality=quality,
            reasoning=reasoning,
        ...
        for call in first["tool_calls"]:
            result = await registry.execute_async(call, allowed_risks=allowed_risks)
            result_message = {
                "role": "tool",
